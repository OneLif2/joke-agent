    ...
    children: [
      ...
      makeTable(
        ...
        [
          ...
          ['LLM (default)', 'google/gemma-4-31b-it via nvidia-ollama-bridge (OpenAI protocol, http://127.0.0.1:11545/v1)'],
          ['LLM (fallback)', 'openai-codex/gpt-5.5 via codex-ollama-bridge (http://127.0.0.1:11540/v1)'],
      ...
      makeTable(
        ...
        [
          ...
          ['Auth', 'Bridge holds NVIDIA_API_KEY internally'],
          ['Fallback bridge', 'codex-ollama-bridge (port 11540, openai-codex/gpt-5.5) — subject to plus-plan quota'],
        ]
      ),
      spacer(),
      p('Why default to gemma:'),
      bullet('No quota cap — avoids the codex usage_limit_reached 429 that throttles longer crawls.'),
      bullet('Free / unlimited via NVIDIA NIM trial endpoint.'),
      bullet('Faster per-call latency (~1–3s vs codex ~7–15s).'),
      bullet('Cantonese understanding is good enough for boundary detection and tagging on most threads.'),
      spacer(),
      p('Python usage (reference):'),
      code('from openai import OpenAI'),
      code(''),
      code('client = OpenAI('),
      code('    base_url="http://127.0.0.1:11545/v1",'),
      code('    api_key="nvidia-bridge",  # any non-empty string'),
      code(')'),
      code(''),
      code('resp = client.chat.completions.create('),
      code('    model="google/gemma-4-31b-it",'),
      ...
      code('curl -sS http://127.0.0.1:11545/v1/models | head -1'),
      p('If the bridge is down: systemctl --user restart nvidia-ollama-bridge'),
      spacer(),
      p('Switch to the codex fallback for higher-quality runs:'),
      code('python3 -m joke_agent test 5 \\'),
      code('    --base-url http://127.0.0.1:11540/v1 \\'),
